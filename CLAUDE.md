...
4. Wire it into the docs — pages are **markdown**, built with fumadocs-mdx:
   create `apps/web/content/docs/components/<name>.mdx` (frontmatter `title`/`description`, optional `badge`; body uses `<ComponentPreview name="<example>" />`, `<ComponentSource name title />`, `<InstallTabs command packages sourceName />`, and `<ApiReference title rows={…} />` islands — see an existing page as template) and add its slug to `apps/web/content/docs/components/meta.json` (page-tree order). API table rows live in `apps/web/lib/api-data/<name>.ts` (imported by both the MDX and the markdown processor). **`apps/web/lib/docs-nav.ts` is still the sidebar + site search index** (`apps/web/components/site-search.tsx`, the ⌘K palette) — any enabled entry there is automatically searchable, so add the nav entry too; never hardcode a separate search index.
