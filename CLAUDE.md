...
6. Every new component or utility must include both a preview image and an Open Graph image. Create **two** preview modules: `apps/web/components/previews/<name>.tsx` exporting `<Name>Preview` (themed — colors via the `preview` token const from `./shared`, CSS variables) and `apps/web/components/previews/og/<name>.tsx` exporting `<Name>Preview` (Satori-safe — the same layout with hardcoded hex colors, since Satori can't resolve CSS variables). Both use plain inline styles, never Tailwind classes. The barrels (`components/previews/index.ts`, `components/previews/og/index.ts`) and the slug→component map (`components/previews/og/__map__.ts`) regenerate automatically via predev/prebuild (`scripts/generate-preview-map.mts`) — never edit them by hand. Then wire the themed name into the catalog island `apps/web/components/mdx/components-catalog.tsx` (renders `/docs/components`). OG images need no per-page route: `apps/web/app/docs/og/[...slug]/route.tsx` reads every docs page's frontmatter title/description plus the matching `og/<name>` preview through `OG_PREVIEWS`. Skipping this step is easy to miss — the doc page works fine without it, but the component never appears in `/docs/components` or gets a social preview image.
