...
4. Wire it into the docs — pages are **markdown**, built with fumadocs-mdx:
   create `apps/web/content/docs/components/<name>.mdx` (frontmatter `title`/`description`, optional `badge`; body uses `<ComponentPreview name="<example>" />`, `<ComponentSource name title />`, `<InstallTabs command packages sourceName />`, and `<ApiReference title rows={…} />` islands — see an existing page as template) and add its slug to `apps/web/content/docs/components/meta.json` (page-tree order). API table rows live in `apps/web/lib/api-data/<name>.ts` (imported by both the MDX and the markdown processor) **and the module must be registered in `apps/web/lib/api-data/index.ts`** (`export * as <camelName> from "./<name>"`) — the `.md`/copy-page endpoint resolves rows through that table, and a missing entry renders API tables with headers but zero rows. **`apps/web/lib/docs-nav.ts` is still the sidebar + site search index** (`apps/web/components/site-search.tsx`, the ⌘K palette) — any enabled entry there is automatically searchable, so add the nav entry too; never hardcode a separate search index.
   - **Markdown copy + `.md` endpoint**: free. `<url>.md` is served from the MDX itself by `apps/web/app/markdown/[[...slug]]/route.ts` (islands expand to code fences, Credits/LastEdited are stripped). No per-page strings or route maps to maintain — just curl the `.md` URL once before pushing to confirm 200.
...
6. Every new component or utility must include both a preview image and an Open Graph image — and so must every guide page in the Components nav group (Charts ships one too); a page whose themed preview is missing or misnamed renders "No preview available" in the sidebar hover tooltip. Create **two** preview modules: `apps/web/components/previews/<name>.tsx` exporting `<Name>Preview` (themed — colors via the `preview` token const from `./shared`, CSS variables) and `apps/web/components/previews/og/<name>.tsx` exporting `<Name>Preview` (Satori-safe — the same layout with hardcoded hex colors, since Satori can't resolve CSS variables). Both use plain inline styles, never Tailwind classes. The themed export name is resolved mechanically by `components/docs-sidebar.tsx` as `${docs-nav title with spaces removed}Preview`, so keep the title's exact casing, acronyms included — "Input OTP" → `InputOTPPreview`, "QR Code" → `QRCodePreview`, "National ID Input" → `NationalIDInputPreview` (not `NationalIdInputPreview`). The barrels (`components/previews/index.ts`, `components/previews/og/index.ts`) and the slug→component map (`components/previews/og/__map__.ts`) regenerate automatically via predev/prebuild (`scripts/generate-preview-map.mts`) — never edit them by hand. Then wire the themed name into the catalog island `apps/web/components/mdx/components-catalog.tsx` (renders `/docs/components`). OG images need no per-page route: `apps/web/app/docs/og/[...slug]/route.tsx` reads every docs page's frontmatter title/description plus the matching `og/<name>` preview through `OG_PREVIEWS`. Skipping this step is easy to miss — the doc page works fine without it, but the component never appears in `/docs/components`, loses its sidebar hover image, and gets no social preview image.

7. Add the slug to `apps/web/app/llms.txt/route.ts`'s hand-maintained group lists (`COMPONENT_GROUPS` / `UTILITY_GROUPS`) — `/llms.txt` is generated exclusively from those arrays and silently omits anything not listed, as happened to Status Button at launch.
...
- `apps/web` — Next.js 16 docs/marketing site. Renders component docs, live previews, and serves the registry JSON from `public/r/`.
  - `apps/web/content/docs/` — all doc pages as MDX (`components/<name>.mdx`, `utilities/<name>.mdx`, plus index/theming/charts), compiled by fumadocs-mdx. Ordering lives in per-folder `meta.json`; one renderer at `components/mdx/docs-doc-page.tsx` serves everything through `app/docs/page.tsx` (root) and `app/docs/[...slug]/page.tsx`. MDX islands are mapped in `apps/web/mdx-components.tsx` (headings, code fences via rehype-pretty-code from `source.config.ts`, `<ComponentPreview>`/`<ComponentSource>` resolving examples by name through the generated `components/examples/__map__.ts`).
...
- **Branches**: `feat/<slug>` for new features, `fix/<slug>` for bug fixes, `docs/<slug>` for documentation-only changes (like this one), `chore/<slug>` for tooling/dependency work. New components use the scoped form `feat/component/<slug>` (e.g. `feat/component/plate-input`). Git forbids `:` in branch names, so the conventional-commit colon form is never used for branches. Slug is a short kebab-case description, not an issue number.
