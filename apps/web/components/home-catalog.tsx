...
export function UtilityPreview({ item }: { item: DocsNavItem }) {
  switch (item.title) {
    ...
    case "Normalize Persian Digits":
      return (
        <div className="flex h-full items-center justify-center gap-3 bg-card font-mono">
          <span className="rounded-md bg-muted px-2 py-1 text-sm">۱۲۳۴۵</span>
    ...
    case "useControllableState":
      return (
        <div className="flex h-full items-center justify-center bg-card p-5">
          <div className="w-40 rounded-lg border border-border bg-background p-3 shadow-sm">
            ...
            <div className="mt-3 flex gap-1">
              {[0, 1, 2].map((dot) => (
                <span
                  key={dot}
                  className={`h-1.5 flex-1 rounded-full ${dot === 1 ? "bg-primary" : "bg-muted"}`}
                />
              ))}
            </div>
          </div>
    ...
    case "useMediaQuery":
      return (
        <div className="flex h-full items-center justify-center gap-2 bg-card p-5">
...
function UtilityCard({ item }: { item: DocsNavItem }) {
  return (
    ...
    >
      <div className="h-44 w-full bg-[var(--card)] text-[var(--card-foreground)]">
      ...
      <div className="p-5">
        ...
        <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
          {utilityDescriptions[item.title]}
        </p>
      </div>
    </Link>
  )
        ...
        <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
          <div className="max-w-xl">
            <p className="mb-3 text-xs font-medium tracking-[0.18em] text-muted-foreground uppercase">
              Small helpers, strong foundations
            </p>
            <h2 className="text-3xl font-semibold tracking-tight sm:text-4xl">
              Utilities for the details that make an interface feel right.
            </h2>
            <p className="mt-3 text-muted-foreground">
              Purpose-built helpers for RTL, responsive behavior, and real
