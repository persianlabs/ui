...
- **A third-party hook reading `Date.now()`/`Math.random()` at render time breaks Next's Cache Components prerender**, even in a `"use client"` example — Server Components still SSR client components on first paint. `@tanstack/react-table`'s `useReactTable()` does this. Deferring via `next/dynamic(..., { ssr: false })` in a `"use client"` wrapper (that option is illegal directly in a Server Component page) did **not** reliably fix it here — the Table page's TanStack example was removed outright rather than keep fighting the prerender heuristics. Prefer avoiding libraries with this behavior in doc examples; if one is unavoidable, verify a full `bun run build` actually succeeds before considering it done, not just `tsc`/dev-mode.
