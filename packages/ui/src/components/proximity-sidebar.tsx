...
type ProximitySidebarProps = {
  activeOffset?: number
  className?: string
  /** Force the text direction instead of auto-detecting it from `<html dir>`. */
  dir?: "ltr" | "rtl"
...
const DASH_PRESETS: Record<SectionKind, DashPreset> = {
  title: { base: 40, bump: 70, thickness: 1, className: "bg-foreground" },
  subtitle: { base: 36, bump: 64, thickness: 1, className: "bg-foreground" },
  section: {
    base: 30,
    bump: 56,
    thickness: 1,
...
/**
 * Dashes always expand outward from the rail edge. Under RTL they grow
 * leftward (away from the content the rail sits against), regardless of side.
 */
const resolveDashOrigin = (side: Side, isRtl: boolean) => {
  if (isRtl) return "right center"
  return `${side === "left" ? "left" : "right"} center`
}
    ...
    >
      <motion.span
        ...
        style={{
          ...
          scaleX,
          transformOrigin: resolveDashOrigin(side, isRtl),
    ...
    >
      <div
        className={`mx-8 flex flex-col ${
          side === "right" || isRtl ? "items-end" : "items-start"
        }`}
        style={{ gap: 8 }}
        onPointerMove={(event) => {
          clearPendingReset()
